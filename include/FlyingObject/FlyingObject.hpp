...
namespace Util {
    class FlyingObject {
    public:
        FlyingObject(std::string name): Name(name){}
        virtual ~FlyingObject() = default;
        virtual void Init(std::shared_ptr<Fighter> fighter,Keys Strength,std::vector<std::shared_ptr<FlyingObject>> EnemyFlyingObjects);
        virtual void Update(std::vector<std::shared_ptr<FlyingObject>> EnemyFlyingObjects,glm::vec2 cameraOffset);
